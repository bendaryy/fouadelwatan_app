  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await fetch(
        `https://fouadelwatan.net/api/customers?search=${searchText}`,
        {
          method: "GET",
          headers: {
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() => {
                  navigation.navigate("CustomersDetails", {
              ...
              >
                <Text style={styles.text}>
                  اسم الشخــص : {item.person_name}
                </Text>
                <Text style={styles.text}>
                  اسم الشركة : {item.company_name}
                </Text>
                <Text style={styles.text}> رقم التليفون : {item.phone1}</Text>
                <Text style={styles.text}>
                  {" "}
                  البريد الإلكترونى : {item.email1}
                </Text>
              </TouchableOpacity>
            )}
